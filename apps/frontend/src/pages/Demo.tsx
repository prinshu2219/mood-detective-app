...
export default function Demo() {
  ...
  function tokenColor(token: string, index: number) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      <DetectiveGuide
        storageKey="guide-demo"
        title="How Computers Do It"
        steps={[
        ...
        primary={{ label: 'Let\'s Try!', onClick: () => {} }}
        secondary={{ label: 'Go to Game', to: '/game' }}
      />
      <h2 className="text-3xl font-bold text-indigo-700">Rule Demo: Word Highlighting</h2>
      <p className="mt-2 text-slate-700">Type a sentence to see how the rule engine scores each token.</p>
      <div className="mt-6 flex gap-2">
        ...
        />
        <button
          onClick={() => { track('analyze_clicked'); setText(text); setAnalysisCount(c=>c+1); if (analysisCount + 1 >= 3) setShowCompletionGuide(true) }}
          className="rounded-xl bg-indigo-600 text-white px-6 py-3"
      ...
      <DetectiveGuide
        storageKey="guide-demo-complete"
        ...
        primary={{ label: 'Play the Game', to: '/game' }}
        secondary={{ label: 'Keep Exploring', onClick: () => {} }}
        visible={showCompletionGuide}
      ...
      <section className="mt-6 rounded-2xl bg-white p-6 shadow">
        <div className="flex items-center gap-3">
          ...
          <span className="text-slate-600">Score: {result.score}</span>
        </div>

        <div className="mt-4 h-2 w-full overflow-hidden rounded-full bg-slate-200" aria-hidden="true">
        ...
        <div className="mt-5 flex flex-wrap gap-2" aria-live="polite">
          {tokens.map((t, i) => (
            <span key={i} className={`rounded-lg px-2 py-1 text-sm ${tokenColor(t, i)}`}>{t}</span>
          ))}
        </div>
      </section>
      ...
      {analysisCount >= 3 && (
        <div className="mt-8 text-center">
          <a
            href="/game"
            className="inline-block rounded-2xl bg-green-600 text-white px-6 py-3 text-lg shadow hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 transition-colors"
          >
            Bored of the rule demo? Let\'s play a game
          </a>
        </div>
      )}
    </main>
