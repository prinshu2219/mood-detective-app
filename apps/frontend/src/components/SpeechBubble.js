...
export function SpeechBubble({ tone = 'neutral', arrow = 'bottom', className = '', children, ...rest }) {
    ...
    return (_jsxs("div", { className: `relative rounded-2xl border-2 p-4 shadow-lg ${toneClass} ${className}`, role: "dialog", "aria-label": "Character speech bubble", ...rest, children: [children, _jsx("div", { className: `absolute after:border-4 after:border-transparent ${arrow === 'top' ? 'bottom-full left-1/2 -translate-x-1/2' :
                    arrow === 'bottom' ? 'top-full left-1/2 -translate-x-1/2' :
                        arrow === 'left' ? 'right-full top-1/2 -translate-y-1/2' :
                            'left-full top-1/2 -translate-y-1/2'} ${arrowClass[arrow]}`, "aria-hidden": "true" })] }));
}
