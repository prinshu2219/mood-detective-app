...
export function getWordSentiment(word) {
    const lowerWord = word.toLowerCase();
    if (happy.includes(lowerWord))
        return 2;
    if (sad.includes(lowerWord))
        return -2;
    if (angry.includes(lowerWord))
        return -2;
    return 0;
}
// Helper function to check if a word is a negation
export function isNegation(word) {
    return negations.includes(word.toLowerCase());
}
// Helper function to check if a word is an intensifier
export function isIntensifier(word) {
