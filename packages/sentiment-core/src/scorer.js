...
function getWordWeight(word) {
    // Check emojis first
    if (word in emojis) {
        return { weight: emojis[word], category: 'emoji' };
    }
...
function calculateConfidence(score, highlights) {
...
export function analyze(sentence) {
    ...
    if (score >= 1) {
        ...
        const hasAngryWords = highlights.some(h => h.weight <= -2 && angry.includes(h.token));
...
export function analyzeDetailed(sentence) {
